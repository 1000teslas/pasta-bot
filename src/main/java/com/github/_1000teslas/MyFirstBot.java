...
public class MyFirstBot extends ListenerAdapter {
    ...
    public static void main(String[] args) throws LoginException, VaultException {
        ...
        var token = vault.logical().read("kv/my-first-bot").getData().get("token");
        JDABuilder.createDefault(token).addEventListeners(new MyFirstBot()).build();
    }
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        switch (content) {
            case "navy seal" -> channel.sendMessage(NAVY_SEAL_PASTA).queue();
        }
    }
}
